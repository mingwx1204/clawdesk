...
mod provider_kind;
pub use provider_kind::{ProviderKind, opencode_go_chat_model_id};
    ...
    pub fn resolve(
        ...
        if let Some(name) = requested {
            ...
            if provider_hint == Some(ProviderKind::OpencodeGo) {
                if let Some(canonical) = opencode_go_chat_model_id(name) {
                    if let Some(model) = self
                        .models
                        .iter()
                        .find(|model| {
                            model.provider == ProviderKind::OpencodeGo
                                && model.id.eq_ignore_ascii_case(canonical.as_str())
                        })
                        .cloned()
                    {
                        return ModelResolution {
            ...
            if provider_hint != Some(ProviderKind::OpencodeGo) {
                if let Some(idx) = self.alias_map.get(&normalize(name)) {
                    if provider_hint.is_none_or(|hint| self.models[*idx].provider == hint) {
                        return ModelResolution {
                            requested: Some(name.to_string()),
